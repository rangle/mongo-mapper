...
var log = null; // mongo-mapper is now an npm package and log gets passed in
 ...
 */
function secureRequestParams(req) {
  // 1) Avoid parameter pollution.
  // See https://www.owasp.org/index.php/Testing_for_HTTP_Parameter_pollution_(OWASP-DV-004)
  try {
    typeCheckValues(req.params);
    typeCheckValues(req.query);
  }
  catch (error) {
    return false;
  }
  ...
  // 2) escape $ and .
  // See http://docs.mongodb.org/manual/faq/developers/#how-does-mongodb-address-sql-or-query-injection
  req.params = escapeKeys(req.params);
  req.query = escapeKeys(req.query);

  return true;
}

function typeCheckValues(map) {
  _.mapObject(map, function(value, key) {
...
function escapeKeyName(pair) {
  var result = pair[0].replace(/\$/g, '\uFF04');
 ...
 */
exports.makeMapper = function (dbConnection, config, loggerIn) {
  log = loggerIn;
  var service = {};
